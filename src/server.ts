...
const processMatchData = (match: any, status: string = 'UPCOMING'): any => {
  if (!match.matchInfo) return null;

  // Extract score data from matchScore if available
  const extractScore = (teamIndex: number): any => {
...
app.post('/api/matches/:id/sync-details', async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
...
app.get('/api/matches/:id/scorecard', async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
...
app.get('/api/matches/:id/historical-scorecard', async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
...
app.get('/api/matches/:id/commentary', async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
...
app.get('/api/matches/:id/overs', async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
